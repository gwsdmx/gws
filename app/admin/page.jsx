  ...
  useEffect(() => {
    async function load() {
      ...
      const [projRes, leadsRes] = await Promise.all([
        ...
        sb.from('leads').select('*').order('created_at', { ascending:false }).limit(10),
      ])
          ...
          return (
            <Card key={k.label} className="p-5">
              <div className="flex items-start justify-between mb-3">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${CLR[k.color]}`}>
                  <Icon className="w-4 h-4"/>
                </div>
              </div>
              <div className="font-display font-bold text-slate-900 text-2xl mb-0.5">
                {loading ? '–' : k.value}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3 max-h-[480px] overflow-y-auto pr-1">
            ...
            {!loading && leads.length === 0 && <p className="text-slate-400 text-sm">Sin leads aún.</p>}
            {leads.map(l => (
              <div key={l.id} className="p-3 rounded-xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-colors">
                <div className="flex items-center gap-2.5 mb-1.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-slate-900 text-sm">{l.nombre}</div>
                    <div className="text-slate-400 text-xs">{l.email}</div>
                  </div>
                  <div className="text-slate-300 text-[10px] shrink-0">
                    {new Date(l.created_at).toLocaleDateString('es-MX')}
                  </div>
                </div>
                {l.mensaje && (
                  <p className="text-slate-600 text-xs leading-relaxed ml-10 bg-white rounded-lg px-3 py-2 border border-slate-100">
                    {l.mensaje}
                  </p>
                )}
              </div>
            ...
            {[
              ...
              return (
                <Link key={item.href} href={item.href}
                  className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 hover:bg-blue-50 hover:border-blue-200 transition-all group">
