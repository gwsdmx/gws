...
export default function Hero() {
  ...
  useEffect(() => {
    const sb = getClient()
    sb.from('hero').select('*').single().then(({ data }) => { if (data) setHero(data) })

    const targets = [{ key:'p', val:50 }, { key:'i', val:4 }, { key:'c', val:100 }]
    targets.forEach(({ key, val }) => {
      let start = 0; const step = val / 40
  ...
  return (
    <section className="relative pt-20 overflow-hidden bg-white">
      <div className="h-1.5 bg-gradient-to-r from-blue-600 via-violet-500 to-cyan-400" />

      {/* Colored blobs */}
      <div className="absolute top-20 right-0 w-96 h-96 bg-blue-100 rounded-full blur-3xl opacity-60 pointer-events-none" />
      <div className="absolute top-40 right-40 w-64 h-64 bg-violet-100 rounded-full blur-3xl opacity-50 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-cyan-50 rounded-full blur-3xl opacity-70 pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8 pt-16 pb-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="animate-fade-up relative z-10">
            ...
            <h1 className="font-display font-bold text-slate-900 leading-tight mb-5" style={{fontSize:'clamp(30px,4.5vw,56px)'}}>
              Sistemas que hacen{' '}
              <span className="relative inline-block">
                <span className="text-blue-600">crecer</span>
                <span className="absolute -bottom-1 left-0 right-0 h-1.5 bg-gradient-to-r from-blue-400 to-cyan-400 rounded-full" />
              </span>
              {' '}tu negocio
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-10 opacity-0 animate-fade-up animate-delay-200" style={{animationFillMode:'forwards'}}>
              <a href="#contacto" className="group inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold text-sm transition-all shadow-lg shadow-blue-600/30 hover:-translate-y-0.5">
                Agenda una demo <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform"/>
              </a>
              <a href="#servicios" className="inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-xl border border-slate-200 text-slate-600 font-medium text-sm hover:bg-slate-50 hover:border-slate-300 transition-all">
                Ver servicios <ChevronRight className="w-4 h-4"/>
              </a>
            </div>

            <div className="flex flex-wrap gap-8 opacity-0 animate-fade-up animate-delay-300" style={{animationFillMode:'forwards'}}>
          ...
          <div className="relative hidden md:block opacity-0 animate-fade-up animate-delay-200 z-10" style={{animationFillMode:'forwards'}}>
            <div className="absolute -top-4 -right-2 z-20 bg-white rounded-xl border border-slate-200 shadow-xl px-3 py-2.5 flex items-center gap-2.5 opacity-0 animate-fade-in animate-delay-500" style={{animationFillMode:'forwards'}}>
              <div className="w-8 h-8 rounded-lg bg-green-100 flex items-center justify-center text-base">✅</div>
              <div><div className="text-green-700 font-semibold text-[10px]">Nuevo proyecto lanzado</div><div className="text-slate-400 text-[10px]">Sistema de inventario · Hoy</div></div>
            </div>
            <div className="absolute -bottom-3 -left-4 z-20 bg-white rounded-xl border border-slate-200 shadow-xl px-3 py-2.5 flex items-center gap-2.5 opacity-0 animate-fade-in animate-delay-400" style={{animationFillMode:'forwards'}}>
              <div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center text-base">🚀</div>
              <div><div className="text-blue-700 font-semibold text-[10px]">Deploy exitoso</div><div className="text-slate-400 text-[10px]">Sistema médico v2.1</div></div>
            </div>
                ...
                <div className="p-4 grid grid-cols-[140px_1fr] gap-3 bg-slate-50/50">
                  ...
                  <div className="flex flex-col gap-2.5">
                    <div className="grid grid-cols-3 gap-2">
                      {[{l:'Ventas',v:'$24,350',s:'↑ 12%',sc:'text-green-600',bg:'bg-green-50'},{l:'Productos',v:'4,821',s:'3 alertas',sc:'text-blue-600',bg:'bg-blue-50'},{l:'Órdenes',v:'147',s:'hoy',sc:'text-violet-600',bg:'bg-violet-50'}].map(k => (
                    ...
                    <div className="bg-white rounded-xl p-3 border border-slate-100">
                      ...
                      <div className="flex items-end gap-1 h-10">
                        {[[40,'#2563eb'],[65,'#60a5fa'],[45,'#2563eb'],[80,'#0ea5e9'],[55,'#2563eb'],[90,'#0ea5e9'],[70,'#6366f1']].map(([h,c],i) => (
                          <div key={i} className="flex-1 rounded-t" style={{height:h+'%',background:c,opacity:0.8}}/>
                        ))}
                      </div>
                    </div>
                  </div>
