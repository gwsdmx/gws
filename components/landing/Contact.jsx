  ...
  return (
    <section id="contacto" className="py-20 sm:py-24 px-5 sm:px-8 bg-[#0f0a1e]">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        {/* LEFT */}
        <div>
          <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-violet-500/15 border border-violet-500/30 text-violet-300 text-xs font-bold tracking-widest uppercase mb-5">
            Contacto
          </span>
          ...
          <div className="space-y-4 mb-8">
            {[
              ...
              <div key={label} className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-white/30 text-[10px] uppercase tracking-wider font-bold">{label}</p>
          ...
          {/* WhatsApp */}
          <div>
            <p className="text-white/25 text-xs font-bold uppercase tracking-widest mb-3">Contáctanos por WhatsApp</p>
            <div className="flex flex-col sm:flex-row gap-2.5">
              {WA.map(wa => (
                <a key={wa.number}
                  href={`https://wa.me/${wa.number}?text=Hola, me interesa una demo de GWS.`}
                  target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2.5 px-5 py-3 rounded-xl bg-[#25D366] hover:bg-[#20bc5a] text-white font-bold text-sm transition-all hover:-translate-y-0.5 shadow-lg shadow-green-500/20">
                  <WhatsAppIcon />{wa.label}
                </a>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-2xl shadow-black/30">
          {status === 'success' ? (
            <div className="text-center py-10">
              ...
              <button onClick={() => setStatus('idle')} className="mt-5 text-violet-600 hover:text-violet-700 text-sm font-bold transition-colors">
                Enviar otro →
              </button>
            </div>
          ) : (
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-slate-600 text-sm font-semibold mb-1.5">Nombre completo *</label>
                  <input name="nombre" type="text" required value={form.nombre} onChange={onChange} placeholder="Tu nombre" className={INPUT}/>
                </div>
                <div>
                  <label className="block text-slate-600 text-sm font-semibold mb-1.5">Correo electrónico *</label>
                  <input name="email" type="email" required value={form.email} onChange={onChange} placeholder="correo@empresa.com" className={INPUT}/>
                </div>
              </div>

              <div>
                <label className="block text-slate-600 text-sm font-semibold mb-1.5">Número de teléfono</label>
                <div className="relative">
                  <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300"/>
                  <input name="telefono" type="tel" value={form.telefono} onChange={onChange} placeholder="+52 771 000 0000"
                    className={`${INPUT} pl-10`}/>
                </div>
              </div>

              <div>
                <label className="block text-slate-600 text-sm font-semibold mb-1.5">¿Qué necesitas? *</label>
              ...
              <button type="submit" disabled={status === 'loading'}
                className="w-full flex items-center justify-center gap-2 px-6 py-3.5 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 disabled:opacity-50 text-white font-bold rounded-xl shadow-lg shadow-violet-500/25 transition-all hover:-translate-y-0.5">
                {status === 'loading'
