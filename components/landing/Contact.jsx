...
const WA = [
  { number:'527731387203',  label:'WhatsApp 1' },
  { number:'5217714140422', label:'WhatsApp 2' },
]
...
export default function Contact() {
  ...
  const onSubmit = async e => {
    ...
    } catch(e) { setErr(e?.message||'Error al enviar.'); setStatus('error') }
  }
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        <div>
          ...
          <div className="space-y-4 mb-8">
            {[{icon:Mail,label:'Correo',val:'gws.dmx@gmail.com'},{icon:Clock,label:'Respuesta',val:'Menos de 24 horas hábiles'}].map(({icon:Icon,label,val})=>(
          ...
          {/* WhatsApp — icon buttons only */}
          <div>
            <p className="text-slate-500 text-xs font-semibold uppercase tracking-widest mb-3">Contáctanos por WhatsApp</p>
            <div className="flex gap-3">
              {WA.map((wa,i) => (
                <a key={wa.number}
                  ...
                  title={wa.label}
                  className="flex items-center gap-2 px-5 py-3 rounded-xl bg-[#25D366] hover:bg-[#20bc5a] text-white font-semibold text-sm transition-all shadow-lg shadow-green-500/20 hover:-translate-y-0.5">
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-xl">
          {status==='success' ? (
            <div className="text-center py-10">
              ...
              <button onClick={()=>setStatus('idle')} className="mt-5 text-blue-600 text-sm font-semibold">Enviar otro →</button>
            </div>
          ) : (
            <form onSubmit={onSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><label className="block text-slate-600 text-sm font-medium mb-1.5">Nombre *</label><input name="nombre" type="text" required value={form.nombre} onChange={onChange} placeholder="Tu nombre" className={INPUT}/></div>
                <div><label className="block text-slate-600 text-sm font-medium mb-1.5">Correo *</label><input name="email" type="email" required value={form.email} onChange={onChange} placeholder="correo@empresa.com" className={INPUT}/></div>
              </div>
              <div><label className="block text-slate-600 text-sm font-medium mb-1.5">Empresa</label><input name="empresa" type="text" value={form.empresa} onChange={onChange} placeholder="Nombre de tu empresa (opcional)" className={INPUT}/></div>
              <div><label className="block text-slate-600 text-sm font-medium mb-1.5">¿Qué necesitas? *</label><textarea name="mensaje" required rows={4} value={form.mensaje} onChange={onChange} placeholder="Describe brevemente tu negocio..." className={`${INPUT} resize-none`}/></div>
